package com.zhiyin.plugins.translator.youdao.utils;
...
public class HttpUtil {
    ...
    private static void addBodyParam(Request.Builder builder, Map<String, String[]> body) {
        if (body == null) {
            return;
        }
    ...
    private static byte[] requestExec(Request request, String expectContentType) {
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            if (response.code() == 200) {
                ...
                return ("request failed, http code: " + response.code()).getBytes(StandardCharsets.UTF_8);
            }
        } catch (IOException ioException) {
            return ("request exec error: " + ioException.getMessage()).getBytes(StandardCharsets.UTF_8);
        }
        return null;
    }
}
